...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("MSGC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("Massagecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MSGC unit input in lnReqAmount, we generate the URI with MSGC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MSGC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
